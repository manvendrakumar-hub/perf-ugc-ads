...
- **PRIVATE (continuity / new-machine):** https://github.com/manvendrakumar-hub/perf-ugc-ads — full real values: KNOWLEDGE_BASE.md (all IDs/recipe), both skills as `.skill` zips, pipeline/ with real config, Avatars/ PNGs, product images, fonts. `.secrets/` gitignored (no keys/tokens committed). Clone this on the new machine, then re-auth per KNOWLEDGE_BASE.md / RUNBOOK. **2026-06-02 update (commit 7eba6ad):** added top-level `README.md` (Mermaid architecture diagram + ad taxonomy A.1/A.2.1/A.2.2/A.3/B + new-machine setup), `docs/memory/` (mirror of the whole continuity memory), `Ads/_Models/{male,female}` + `_product_src` images. `.gitignore` hardened to also exclude `Music/` `*.mp3`, `Female face ref/` (real-person PII), `Video ref/` (3rd-party ref ads), `**/_scratch/`, `.claude/`, `*.lock`. **Decision 2026-06-02: user chose to KEEP IT PRIVATE** (declined making public — would have permanently exposed internal Sheet/Drive/GCP IDs; history verified to contain NO secret files). To go public later: redact Sheet ID `1jTyw…`, Drive parent `1L_CIGT9y…`, GCP `130084846397`/`spring-market-498112-f8` first.
